# Xpenzo - Claude Code Memory File
...
## Groups & Splits (planned feature — Splitwise-style)

Xpenzo will support **bill splitting with friends/groups** ("who owes whom" + settle-up).
Key facts so a cold session doesn't re-derive them:

- **Decision: Option B — a SEPARATE subsystem from Family.** Family = one shared household
  (full visibility, shared budgets). Groups = many ad-hoc circles you split with (trip,
  flatmates, couple). They do NOT share collections/tables.
- **Status:** spec'd, NOT built. It is **post-MVP, Phase 10** — must not delay the MVP critical path.
- **Canonical spec locations (keep these in sync):**
  - PRD feature: `docs/08 - Product Requirements Document (PRD).md` → **§5.8 / F8**
  - Firestore data model + rules + indexes + Cloud Functions: `docs/04 - Backend Schema Documentation.md` → **§3.8, §5.3**
  - Room entities (Tables 12–17) + DB v2 migration: `docs/01 - Technical Requirements Document (TRD).md` → **§5.2–5.3**
  - User flows: `docs/03 - App Flow Documentation.md` → **§9**
  - Build plan: `docs/PROJECT_COMPLETION_ROADMAP.md` → **Phase 10**
- **Data model:** Room Tables 12–17 = `groups, group_members, split_expenses, split_shares, settlements, friends`; Firestore `/groups/{id}/{info,members,expenses,settlements,balances}` + `/users/{uid}/friends/*`.
- **Split types:** EQUAL / EXACT / PERCENT / SHARES. **Debt simplification:** min-cash-flow (≤ N-1 settlements). **Settle-up:** CASH or `upi://pay` deep link, payee confirms.
- **Accounting rule:** a split does NOT create a second expense — the payer's transaction stays their expense and gets a nullable `reimbursable_amount`; budgets net out the reimbursed portion.
- **🔒 Privacy:** split-partner identity / group membership / settlements MUST NEVER enter the ML loop (`ml_corrections`). Only the local user's own normalized merchant flows to ML, unchanged.
